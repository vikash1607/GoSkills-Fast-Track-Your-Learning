    ...
    >
      <div className="flex w-11/12 max-w-maxContent items-center justify-between">
        ...
        <Link to="/" className="relative">
          <img src={logo} alt="Logo" width={160} height={32} loading="lazy" className="ml-6 md:ml-8 lg:ml-2 " />
        </Link>
        </div>
        ...
        <div className="hidden items-center gap-x-4 md:flex">
          ...
          {token === null && (
            <Link to="/login">
              <button className="rounded-[8px] border border-richblack-700 bg-richblack-800 px-3 py-2 text-xs text-richblack-100">
                Log in
              </button>
            </Link>
          )}
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="flex items-center gap-4 md:hidden mt-4 mb-4">
          {user && user?.accountType !== ACCOUNT_TYPE.INSTRUCTOR && (
            <Link to="/dashboard/cart" className="relative">
              <AiOutlineShoppingCart className="text-2xl text-richblack-100" />
              {totalItems > 0 && (
